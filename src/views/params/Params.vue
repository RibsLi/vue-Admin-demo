    ...
    <el-card>
      ...
      <el-tabs v-model="activeName" @tab-click="tabClick">
        ...
        <el-tab-pane label="动态参数" name="many">
          <el-button type="primary" :disabled="isDisabled" @click="addParamsDialog = true">添加动态参数</el-button>
          <!-- 列表信息 -->
          <el-table :data="paramsList" border height="57vh" highlight-current-row stripe style="width: 100%">
            <!-- 参数列表扩展 -->
            ...
            <el-table-column prop="attr_name" label="参数名称" header-align="center" />
            <el-table-column label="操作" header-align="center" width="173px">
              <template v-slot:default="handle">
                <!-- 编辑按钮 -->
                <el-button type="primary" icon="el-icon-edit" size="small" @click="editClick(handle.row.attr_id)"></el-button>
                <!-- 删除按钮 -->
                <el-button type="danger" icon="el-icon-delete" size="small" @click="removeClick(handle.row.attr_id)"></el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-tab-pane>
        ...
        <el-tab-pane label="静态属性" name="only">
          <el-button type="primary" :disabled="isDisabled" @click="addParamsDialog = true">添加静态属性</el-button>
          <!-- 列表信息 -->
          <el-table :data="paramsList" border height="58.8vh" highlight-current-row stripe style="width: 100%">
            <!-- 参数列表扩展 -->
            <el-table-column type="expand" header-align="center" align="center">
              <template v-slot:default="tag">
                <el-tag closable v-for="(item, index) in tag.row.attr_vals" :key="index">
...
import { getParams, addParams, getParamsInfo, setParams, deleteParams } from "network/params"
  ...
  methods: {
    ...
    submitAddParamsForm() {
      this.$refs.addParams.validate((valid) => {
        if (valid) {
          addParams(this.newCat[2], this.addParams.attr_name, this.activeName, '').then(res => {
            console.log(res);
    ...
    // 编辑参数按钮点击事件
    editClick(attr_id) {
      getParamsInfo(this.newCat[2], attr_id, this.activeName, '').then(res => {
        // console.log(res);
    ...
    submitEditForm() {
      this.$refs.paramsInfo.validate((valid) => {
        if (valid) {
          setParams(this.newCat[2], this.paramsInfo.attr_id, this.paramsInfo.attr_name, this.activeName, '').then(res => {
            // console.log(res);
